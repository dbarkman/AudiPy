# AudiPy Database Schema

## 📊 Analysis Summary

Based on the API dump:
- **User Library Books**: 142 fields (rich, detailed data)
- **Catalog Books**: 31 fields (basic search data)
- **Strategy**: Store everything from user library, supplement with catalog data
...
```sql
-- Create database
CREATE DATABASE audipy CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- Create user (replace with your preferred credentials)
CREATE USER 'audipy'@'localhost' IDENTIFIED BY 'cpc3KCa8dffm2PcoFBAe';
GRANT ALL PRIVILEGES ON audipy.* TO 'audipy'@'localhost';
FLUSH PRIVILEGES;

-- Use the database
USE audipy;

-- Run all the CREATE TABLE statements above in order
```
...
OAUTH_SECRET_KEY=your_oauth_secret_key_here
...
ENCRYPTION_KEY=your_encryption_key_for_audible_tokens
...
### Indexing Strategy
- **Primary searches**: Title, author, narrator, series
- **User queries**: User libraries, wishlists, suggestions
- **Price tracking**: Price history and alerts
- **Full-text search**: Book descriptions and content

### Data Partitioning (Future)
- Partition `user_analytics` by date
- Partition `book_prices` by marketplace and date
- Partition `sync_jobs` by user and date

### Caching Strategy
- **Redis cache** for frequently accessed book data
...
2. **Data import**: Bulk import from API dumps
3. **User migration**: Migrate from CLI to web usage
4. **Performance optimization**: Add indexes as needed
5. **Feature expansion**: Add new tables for social features

This schema captures all 142 fields from the Audible API while maintaining good relational design and performance characteristics! 